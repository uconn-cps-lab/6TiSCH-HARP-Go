...
void initChildren()
{
    ...
    for (int i = 0; i < 6; i++)
    {
        ...
        HARP_children[i].sp_rel[0].ts_start = 0;
        HARP_children[i].sp_rel[0].ts_end = 0;
        HARP_children[i].sp_rel[0].ch_start = 0;
        HARP_children[i].sp_rel[0].ch_end = 0;
    }
}
...
void initSelfSP()
{
    ...
    {
        HARP_self.sp_abs[layer].ts_start = 10;
        HARP_self.sp_abs[layer].ts_end = 30;
        HARP_self.sp_abs[layer].ch_start = 5;
...
uint8_t interfaceComposition()
{
    ...
    HARP_children[0].sp_rel[layer].ch_end = 0 + HARP_children[layer].iface[layer].ch;
    ...
    printf("child #%d's relative sub-partition {%d, %d, %d, %d}\n", HARP_children[0].id, HARP_children[0].sp_rel[0].ts_start, HARP_children[0].sp_rel[0].ts_end,
           HARP_children[0].sp_rel[0].ch_start, HARP_children[0].sp_rel[0].ch_end);
    ...
    printf("Composited interface: {%d, %d}\n", HARP_self.iface[layer].ts, HARP_self.iface[layer].ch);
...
uint8_t subpartitionAllocation()
{
    ...
    for (int i = 0; i < 6; i++)
    {
        ...
        printf("child #%d's absolute sub-partition {%d, %d, %d, %d}\n", HARP_children[i].id, HARP_children[i].sp_abs[0].ts_start,
               HARP_children[i].sp_abs[0].ts_end, HARP_children[i].sp_abs[0].ch_start, HARP_children[i].sp_abs[0].ch_end);
    }
    return 0;
}
